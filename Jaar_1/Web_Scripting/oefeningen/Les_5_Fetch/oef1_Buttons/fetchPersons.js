...
function loaded() {
    ...
    let buttonGetPersonByName = document.getElementById('button_get_persons_by_name');
    buttonGetPersonByName.addEventListener("click", handleGetPersonByName);

    let buttonPutPost = document.getElementById('button_put_person');
    buttonPutPost.addEventListener("click", handlePutPersonAsync);
}
...
async function handlePutPersonAsync(){
    let url = 'http://localhost:3000/persons/';
    let output = document.getElementById("div_output");
    let name = document.getElementById("txt_name").value;
    let id = document.getElementById("txt_id").value;
    ...
    try {
        ...
        let data = [];
        data.push([updatedPerson.id, updatedPerson.name]);
        let table = makeTable(data);
        output.appendChild(table);
    } catch(err) {
        output.appendChild(document.createTextNode(error));
