  ...
  if (isNaN(c)) {
    ...
    if (c % 1 !== 0) {
      ...
      if (c > 999 || c < 2) {
        ...
        paragraph = `<div class='row p-3'>
          <span class='col text-danger'>Subnet Names:</span>
          <span class='col text-danger'>Number of hosts:</span><br />
        </div>`;
        for (var b = 1; b <= c; b++) {
        ...
        final = `<div class='form-group' <form> ${paragraph} </form> </div>`;
        document.getElementById("nets").innerHTML = c;
        document.getElementById("subnet_pargraph").innerHTML = final;
        document.getElementById("haveTohide").style.display = "none";
      }
    }
  }
}
function vlsm() {
  ...
  // validation of input network
  if (!validate(h)) {
    document.getElementById("not_valid_ip").innerHTML =
      "<b>This does not seem like a valid network</b>";
    return;
  }

  document.getElementById("not_valid_ip").innerHTML = "";
  var n = return_slash(h);
  console.log("After slashing", n);

  var g = find_hosts(n);
  console.log("After finding hosts", g);

  var m = return_ip(h);
  console.log("After returning ip", m);

  var q = find_mask(n);
  console.log("After Finding mask", q);
  ...
  var s =
    ...
    " has " +
    g +
    " hosts.<br>Your subnets need " +
    a +
    " hosts.</p>";
  var t =
    "<table border='1' class='table table-bordered mt-3'><thead class='thead-dark'><td>Name</td><td>Hosts Needed</td><td>Hosts Available</td><td>Unused Hosts</td><td>Network Address</td><td>Slash</td><td>Mask</td><td>Usable Range</td><td>Broadcast</td><td>Wildcard</td></thead>";
  var b = c;
  var k = 0;
  ...
  if (k > g + 2) {
    s +=
      "<span class='alert alert-danger mb-5 p-2'>Looks like those subnets will not fit into that network, but here is something else that may work for you:</span><br>";
  }
  t = s + t;
